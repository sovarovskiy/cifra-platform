/**
 * Копирует «ОГЗ.pdf» с рабочего стола и конвертирует страницы в PNG для главной.
 * Запуск: npm run home-ogz
 */
import fs from "fs";
...
const assetDir = path.join(rootDir, "public", "home-ogz");
const imageDir = path.join(assetDir, "images");
const pdfDest = path.join(assetDir, "ogz.pdf");
const manifestPath = path.join(rootDir, "data", "home-ogz.json");

const SOURCE_FILE = "ОГЗ.pdf";
...
const SOURCE_PATHS = [
  ...
  "C:\\Users\\Windows\\OneDrive\\Рабочий стол\\ОГЗ.pdf",
];
...
async function convertPdfToImages(pdfPath) {
  ...
  for await (const image of document) {
    ...
    imageUrls.push(`/home-ogz/images/${filename}`);
    pageNum++;
  }
